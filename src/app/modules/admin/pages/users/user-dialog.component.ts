...
interface DialogData { user?: User; }
  ...
  template: `
    ...
    <mat-dialog-content>
      <form [formGroup]="form" class="dialog-form">
        ...
        <div class="dialog-info" *ngIf="!isEdit && !error()">
  ...
  ngOnInit(): void {
    ...
    if (this.data?.user) {
      ...
      this.form.patchValue({
        ...
        es_kiosk:   u.es_kiosk ?? false,
      });
      const isAdmin = u.rol === 'admin_ondra';
      this.showClientField.set(!isAdmin);
      if (!isAdmin) this.form.get('cliente_id')!.setValidators(Validators.required);
  ...
  protected submit(): void {
    ...
    if (this.isEdit) {
      this.usersService.update(this.data!.user!.id, {
        ...
        error: err => { this.loading.set(false); this.error.set(err?.error?.error ?? 'Error al actualizar el usuario.'); },
      });
    } else {
