...
interface DialogData { password: string; title?: string; }
...
@Component({
  ...
  template: `
    <div class="pwd-dialog">
      ...
      <h2 class="pwd-dialog__title">{{ data.title ?? 'Contraseña generada' }}</h2>
      ...
      <div class="pwd-dialog__box">
        ...
        <button mat-icon-button class="pwd-dialog__copy" (click)="copy()" [matTooltip]="copied() ? '¡Copiada!' : 'Copiar'">
          <mat-icon>{{ copied() ? 'check' : 'content_copy' }}</mat-icon>
        </button>
      </div>

      <mat-dialog-actions align="end">
        <button mat-flat-button mat-dialog-close color="primary" class="pwd-dialog__close">
          Entendido
        </button>
      </mat-dialog-actions>
  ...
  styles: [`
    ...
    .pwd-dialog__icon {
      ...
      margin:          0 auto 16px;
      mat-icon { color: #4dd0e1; font-size: 26px; width: 26px; height: 26px; }
    }
    ...
    .pwd-dialog__warn {
      ...
      margin:      0 0 20px;
      mat-icon { font-size: 16px; width: 16px; height: 16px; color: rgba(255,193,7,0.85); }
    }
